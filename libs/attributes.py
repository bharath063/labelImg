...
try:
    ...
    from PyQt5.QtWidgets import QLineEdit, QCheckBox, QRadioButton, QComboBox, QWidget, QFormLayout, QPushButton, QDockWidget
except ImportError:
    from PyQt4.QtGui import QImage, QFormLayout
...
class AttributesWidgets():
    ...
    def get_widget_value( self, widget ):
        if type(widget) is QLineEdit:
            ...
            # this is setting value to "no" always on initial load from file.
            if widget.isChecked():
                return "yes"
            else:
                return "no"
    ...
    def build_widget( self, key, definition ):
        ...
        if type == "text":
            ...
            # widget.setText( definition['text'] )
            if action is not None:
                widget.toggled.connect( action )
...
class AbstractAttributesWidgets( AttributesWidgets ):
    ...
    def loadLabelAttributes( self, label_attributes ):
        ...
        # TODO Need to investigate why this requires cloning
        # Turns out need to wait for the init of all widgets inside actions before actually changing values
